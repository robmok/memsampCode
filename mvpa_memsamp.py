...
#just to get the df structure (Better way to do this? maybe just an empty df? line 44)
condPath=os.path.join(bidsDir, 'sub-01', 'func','sub-01' + '_task-memsamp_run-01_events.tsv')

df=pd.read_csv(condPath, sep='\t')

dfCond=pd.DataFrame(columns=list(df)) 
for iRun in range(1,4):
    ...
    #df.loc[:,'run2']=pd.Series(np.ones((len(df)))*iRun,index=df.index)
    ...
    for iCond in conds:
        for iTrial in range(1,8): #calculate cope number
            ...
            imPath.append(os.path.join(featDir, 'sub-' + subNum + '_run-0' + str(iRun) +'_trial_T1_fwhm0.feat','stats','cope' + (str(copeNum)) + '.nii.gz'))
            copeNum=copeNum+1
    df['imPath']=pd.Series(imPath,index=df.index) # temp df for this run
